...
function resnameCheck() {
	$('#name_msg-res').text("");
	const pattern = /^[가-힣]{2,10}$/;
	return check($('#name-res').val(),pattern,"이름은 한글 2~10글자입니다",$("#name_msg-res"));
}
...
function respersonalIdCheck() {
	...
	const pattern = /^(?:[0-9]{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[1,2][0-9]|3[0,1]))$/;
	return check($('#personalId-res').val(),pattern,"주민등록번호 형식에 맞게 입력해주세요",$("#personalId_msg-res"));
}
...
function respersonalId2Check() {
	$('#personalId_msg-res').text("");
	const pattern = /^[1-8][0-9]{6}$/;
	return check($('#personalId2-res').val(),pattern,"주민등록번호 형식에 맞게 입력해주세요",$("#personalId_msg-res"));
}
...
function resemailCheck() {
	$('email_msg-res').text("");
	const pattern = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i;
	return check($('#email-res').val(),pattern,"이메일을 정확하게 입력하세요",$('#email_msg-res'));
}
...
function restelCheck() {
	$('tel_msg-res').text("");
	const pattern = /^01([0|1|6|7|8|9])(?:[0-9]{4})(?:[0-9]{4})$/;
	return check($('#tel-res').val(),pattern,"휴대전화 형식에 맞게 입력하세요",$('#tel_msg-res'));
}
...
function resusernameCheck() {
	...
	return check(value,/^[a-z0-9]{7,10}$/, "아이디는 문자, 숫자의 조합 7~10자입니다",$('#username_msg-res'));
}
...
function respasswordCheck() {
	...
	const pattern = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/;
	return check($('#password-res').val(), pattern, "비밀번호는 문자, 숫자, 특수문자의 조합 최소 8자리입니다.", $("#password_msg-res"));
}
...
function respassword2Check() {
	...
	if(value!=$('#password-res').val()) {
		$('#password2_msg-res').text("비밀번호가 일치하지 않습니다.").attr("class","fail");
		return false;
	}
}
...
$(document).ready(function() {
	...
	$('#email-res').blur(async function() {
		...
		try {
			...
			$("#email_msg-res").text("사용가능한 이메일입니다.").attr("class","success");
		} catch(err) {
	...
	$('#username-res').blur(async function() {
		...
		try {
			await $.ajax('/hotel/member/check/username?username='+$('#username-res').val());
			$("#username_msg-res").text("사용가능한아이디입니다.").attr("class","success");
		} catch(err) {
	...
	$('#join-res').click(async function() {
		const resresult = resnameCheck() && respersonalIdCheck() && respersonalId2Check() && resemailCheck() && restelCheck() && resusernameCheck() && respasswordCheck() && respassword2Check();
		if(resresult==false) {
			alert("입력사항을 다시한번 확인해주세요.");
			return false;
		}
